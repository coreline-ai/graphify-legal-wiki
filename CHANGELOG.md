...
- Added `graphify clone <github-url>` for cached GitHub repository cloning.
- Added `graphify merge-graphs` for cross-repository graph composition.
- Added shrink guard and `build_merge()` for safer incremental graph updates.
- Improved duplicate node deduplication and graphify-out source-scan exclusion.
- Expanded platform install support and docs for Codex/OpenCode/Cursor/Gemini/VS Code/Antigravity/Kiro workflows.
